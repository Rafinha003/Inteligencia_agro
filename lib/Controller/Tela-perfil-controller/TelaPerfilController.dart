...
class TelaPerfilController {
  ...
  Future<List<String>> obterTipoTransacao() async {
    try {
      QuerySnapshot snapshot =
          await _firestore.collection('TipoTransacao').get();
  ...
  String formatarValor(String valorOriginal) {
    ...
    if (somenteNumeros.length == 2 && !somenteNumeros.startsWith('0')) {
    ...
    } else if (somenteNumeros.length == 3 &&
        somenteNumeros.startsWith('0')) {
      String reais = "0";
      ...
      String reais =
          somenteNumeros.substring(0, somenteNumeros.length - 2);
      String centavos =
          somenteNumeros.substring(somenteNumeros.length - 2);
      valorDouble = double.parse("$reais.$centavos");
    }
  ...
  Future<String> converterImagemParaBase64(Uint8List bytes) async {
    ...
    if (imagemDecode != null) {
      img.Image imagemRedimensionada =
          img.copyResize(imagemDecode, width: 300);
  ...
  Future<List<Map<String, dynamic>>> obterPropostasDoUsuario() async {
    try {
      ...
      for (var doc in propostasSnapshot.docs) {
        ...
        final compradorRef = await _firestore
            .collection('Usuario')
            .doc(proposta['uidComprador'])
            .get();
        final compradorData =
            compradorRef.data() as Map<String, dynamic>?;
        ...
        if (itemData != null && compradorData != null) {
          propostas.add({
            ...
            'telefoneComprador':
                compradorData['telefone'] ?? 'Não informado',
          });
        }
      }
      return propostas;
  ...
  Future<void> atualizarStatusProposta(
      String idProposta, String status) async {
    try {
